...
%function to check whether any annotations are in mismatched tiers
%and/or if there are non-sensical or missing annotations
...
for j = 1:numel(TierInfoTable.StartTimeRef)
    if contains(TierInfoTable.TierTypeVec{j},'Infant Voc Type') %if infant voc type tier
        if sum(strcmpi(strtrim(TierInfoTable.Annotation{j}),{'X','R','C','L'})) ~= 1 %if, after eliminating white space, the infant annotation 
            %is not one of these types; we eliminate white space from
            %the comparison because we can go in and edit white space
            %and tab characters
            %we use contains instead of strcmpi becuase white space and
            %random non-text characters are things we can go in and edit
            %ourselves
