...
# 7.) Save all changes
# 8.) Launch Terminal
# 9.) Navigate to directory where "F2_RunFolderReadEafFilesAsText.sh" is located
# 10.) Run this file (sh F2_RunFolderReadEafFilesAsText.sh )

cd ~/Google\ Drive/research/vocalisation/Pre_registration_followu/Data/HUMLabelData/EAFFiles/
for eaffile in *.eaf         #go through eaf files
	do outfile=`echo $eaffile | sed 's/\.eaf/EAF\.txt/g'`;       
	   perl ~/Google\ Drive/research/vocalisation/Pre_registration_followu/Sample_its_files/lena-its-tools-master/AdditionalCodeByRVPS/HUMLabelDataProcessing/ReadEafFilesAsText.pl $eaffile $outfile
	   done
