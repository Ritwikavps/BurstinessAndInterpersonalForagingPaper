...
%Inputs: - StartTime: start time vector for all AN and CHN (CHNSP, CHNNSP) vocs
        ...
        %- OtherType: Responder type(s) for which we want response data
                %For example, if we want adult responses to CHNSP ONLY,
                %then SpeakerType would be CHNSP and OtherType would be AN
        %- NAType: Voc type(s) which can trigger NA response. For example,
            %if we want both CHNSP and CHNNSP following a CHNSP voc within 1 s
            %without an intervening vocalisation to trigger an NA. For now,
            %we will set NAType for AN responses to CHN(SP or NSP) as all
            %CHN types
...
for i = 1:numel(StartTime)-1 %(for all but the last voc)
    ...
    if contains(SpeakerVec{i},SpeakerType) == 1 %if current voc is relevant speaker type
        ...
        if (contains(NextSpeaker,NAType)) && (CurrentEndToNextStartTime <= IntervalTime)
            %If next voc is within the time limit AND is the NAType speaker type, response is NaN (NA)
