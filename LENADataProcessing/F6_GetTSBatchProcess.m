...
parfor i = 1:numel(N) %go through each subfolder; parallelising for faster processing
    ...
    desiredpath = strcat('/Users/ritwikavps/Library/CloudStorage/Box-Box/IVFCR Coding/LENAExports/',N{i});
    cd(desiredpath)
    ...
    for j = 1:numel(newdir) %go through each wavefile
        ...
        SegmentsFile = strcat('/Volumes/GoogleDrive/My Drive/research/IVFCRAndOtherWorkWithAnne/Pre_registration_followu/Data/LENAData/Segments/',NameRoot,'_Segments.csv'); %to use in downstream function
        TSfile = strcat('/Volumes/GoogleDrive/My Drive/research/IVFCRAndOtherWorkWithAnne/Pre_registration_followu/Data/LENAData/TimeSeries/',NameRoot,'_TS.csv'); %to check if TS file exists
        ...
        if (isfile(SegmentsFile) == 1) && (isfile(TSfile) == 0)%continue ONLY if the corresponding segments file exists
            ...
            outFile = strcat('/Volumes/GoogleDrive/My Drive/research/IVFCRAndOtherWorkWithAnne/Pre_registration_followu/Data/LENAData/TempTS/',NameRoot,'_TS.csv');
            speakers = {'CHNSP','CHNNSP','FAN','MAN'};
            getAcousticsTS(SegmentsFile,wavFileDir,wavfilebase,outFile,speakers);

            movefile(outFile,'/Volumes/GoogleDrive/My Drive/research/IVFCRAndOtherWorkWithAnne/Pre_registration_followu/Data/LENAData/TimeSeries');
