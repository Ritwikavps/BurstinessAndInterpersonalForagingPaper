...
parfor i = 1:numel(N) %go through each subfolder; parallelising for faster processing
    ...
    for j = 1:numel(newdir) %go through each wavefile
        ...
        SegmentsFile = strcat('/Volumes/GoogleDrive/My Drive/research/IVFCRAndOtherWorkWithAnne/Pre_registration_followu/Data/LENAData/A2_Segments/',NameRoot,'_Segments.csv'); %to use in downstream function
        TSfile = strcat('/Volumes/GoogleDrive/My Drive/research/IVFCRAndOtherWorkWithAnne/Pre_registration_followu/Data/LENAData/A4_TimeSeries/',NameRoot,'_TS.csv'); %to check if TS file exists
        ...
        if (isfile(SegmentsFile) == 1) && (isfile(TSfile) == 0)%continue ONLY if the corresponding segments file exists
            ...
            movefile(outFile,'/Volumes/GoogleDrive/My Drive/research/IVFCRAndOtherWorkWithAnne/Pre_registration_followu/Data/LENAData/A4_TimeSeries');
