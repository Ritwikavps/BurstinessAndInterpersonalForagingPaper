function ResponseVector = ComputeResponseVector(StartTime, EndTime, SpeakerVec, SpeakerType, OtherType, IntervalTime)
                %ComputeResponseVector(SpeakerStartTime,SpeakerEndTime, OtherStartTime, IntervalTime)
...
%Inputs: - StartTime: start time vector for all AN and CHN (CHNSP, CHNNSP) vocs
        ...
        %- SpeakerVec: Vector containing speaker labels (CHNNSP, CHNSP,
                %FAN and MAN)
        %- SpeakerType: Speaker type for which we want response data. 
        % - OtherType: Responder type for which we want response data
                %For example, if we want adult responses to CHNSP ONLY,
                %then SpeakerType would be CHNSP and OtherType would be AN
        %- IntervalTime: See Rule to determine responses above

%The basic algorithm is to step through all vocs, and for each SpeakerType
%voc, find the following voc to see if response = Y, NA or N. 
...
for i = 1:numel(StartTime)-1 %(for all but the last voc)
    ...
    if contains(SpeakerVec{i},SpeakerType) == 1 %if current voc is relevant speaker type
        ...
        if (CurrentEndToNextStartTime > IntervalTime)
            ResponseVector(i) = 0;
        elseif (contains(NextSpeaker,OtherType)) && (CurrentEndToNextStartTime <= IntervalTime) 
            %the interval time condition is redundant here and in the next condition, but I am keeping it for my own anxiety reasons!
            %If next voc is within the allowed time limite AND it is the
            %other speaker type, response is YES (1)
            ResponseVector(i) = 1;
        elseif (contains(NextSpeaker,SpeakerType)) && (CurrentEndToNextStartTime <= IntervalTime)
            %Otherwise, if next voc is within the time limit AND is the same speaker type, response is NaN (NA)
