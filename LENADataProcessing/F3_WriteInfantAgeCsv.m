...
if isempty(aa) %chekc if there are .csv files
    error('There are no .csv files ending with InfantAge.csv')
end
...
opts = detectImportOptions('ItsFileDetails.csv');
opts = setvartype(opts, 'InfantCode', 'string');
ItsFileTab = readtable('ItsFileDetails.csv',opts);
...
for i = 1:numel(aa) %go through .csv files
    ...
    FileNameRoot{i,1} = strrep(aa(i).name,'InfantAge.csv',''); %get file name root
    
    for j = 1:size(ItsFileTab,1) %loop through the rows to match file names
        if contains(ItsFileTab{j,1},FileNameRoot{i,1}) %if fileroot matches
