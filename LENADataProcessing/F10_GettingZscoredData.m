...
destinationpath = '/Volumes/GoogleDrive/My Drive/research/IVFCRAndOtherWorkWithAnne/Pre_registration_followu/Data/LENAData/MatFiles/';

%go to folder with Joined Acoustics data
cd '/Volumes/GoogleDrive/My Drive/research/IVFCRAndOtherWorkWithAnne/Pre_registration_followu/Data/LENAData/AcousticsTSJoinedwPausesAndResponses'

TSFiles = dir('*_AcousticsTSJoined.csv');

opts = detectImportOptions('MergedTSAcousticsDetails.csv'); %make sure to read in infant code as string
opts = setvartype(opts, 'InfantID', 'string');
DataDetails = readtable('MergedTSAcousticsDetails.csv',opts);
%var names: {'FileNameRoot'}    {'InfantAge'}    {'InfantID'}
...
for i = 1:numel(TSFiles)
    ...
    for j = 1:numel(DataDetails.FileNameRoot)
        if strcmp(FNameFromTab,DataDetails.FileNameRoot{j}) == 1
            ...
            InfantAge(i,1) = DataDetails.InfantAge(j);
        end
    end
end
