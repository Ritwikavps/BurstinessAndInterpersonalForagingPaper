...
#Ritwika VPS, Feb 2021 (adapted from code by Anne Warlaumont)
...
for itsfile in *.its 
	...
	do outfile=`echo $itsfile | sed 's/\.its/InfantAge\.csv/g'`; 
	   ...
	   perl ~/Google\ Drive/research/vocalisation/Pre_registration_followu/Sample_its_files/lena-its-tools-master/AdditionalCodeByRVPS/LENADataProcessing/GetInfantAgeFromItsFile.pl $itsfile $outfile
	   done
