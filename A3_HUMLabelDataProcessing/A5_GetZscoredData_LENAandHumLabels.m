...
% LENA: 'wavfile', 'speaker', 'start', 'xEnd', 'duration, 'meanf0', 'dB', 'SubrecEnd', 'FileNameUnMerged'
% HUM: 'speaker', 'start', 'xEnd', 'duration, 'meanf0', 'dB', 'Annotation', 'FnameUnmerged', 'SectionNum' 
...
%function 1: this function outputs all the necessary bits and pieces for the z-scoring. The way we do the z-scoring across the entire LENA and human-listenr labelled data is as follows:
...
%Inputs: - TSfiles: directory information from dir() on the LENA or human-listener labelled time series directories.
        ...
        %- AddToIndex: the index modifier to keep track of whether we are adding LENA or human-labelled data to the vector containing all data smushed together. So, given that i is the 
            ...
            % to the i-th LENA file in the LENA TA directory. However, since we are adding the human-listener data AFTER the LENA data, to properly keep track of the data, if j is the
...
function [LENAandHum_TScell,dB_ChnAn,logf0_ChnAn,logDur_ChnAn,NumelChnAn] = GetInputsForZscoring(TSfiles,LENAandHum_TScell,AddToIndex,dB_ChnAn,logf0_ChnAn,logDur_ChnAn,NumelChnAn)
    for i = 1:numel(TSfiles) %go through list of files
        ...
        dB_ChnAn = [dB_ChnAn; TSTab.dB]; logf0_ChnAn = [logf0_ChnAn; log(TSTab.meanf0)]; logDur_ChnAn = [logDur_ChnAn; log(TSTab.duration)]; %concatenate dB, F0, duration:
        NumelChnAn(AddToIndex + i) = numel(TSTab.dB); %get number of elements in AN + all CHN vectors
    end
end

%function 2: this function picks out each data table from LENAandHum_TScell, removed the raw acoustics data columns, and replaces them wityh teh z-scored ones, and saves the file.
