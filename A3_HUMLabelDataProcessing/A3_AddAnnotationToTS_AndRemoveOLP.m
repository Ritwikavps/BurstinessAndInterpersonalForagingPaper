...
for i = 1:numel(TSFnrootVec) %go through TSfiles, match to correpsonding label file, add annotation tags
    ...
    %     FracDurErr = (TabToSave.duration-(TabToSave.xEnd-TabToSave.start))./TabToSave.duration;
    %     if ~isempty(FracDurErr(FracDurErr > 0.001))
    %         TSFnrootVec{i}
    %         FracDurErr(FracDurErr > 0.001)
    %     end
                                    
    %remove VocIndex and FileName 
    TabToSave = removevars(TabToSave,{'wavfile','VocIndex' });
