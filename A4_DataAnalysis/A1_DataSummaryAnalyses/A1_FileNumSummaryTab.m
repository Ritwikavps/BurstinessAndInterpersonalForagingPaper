...
for i = 1:numel(DatasetType)
    Curr_NumFilesByAge{i} = GetNumFilesByAge(AgeMonth,MetadataCell{i}); %Get number of files for each age (3, 6, 9, and 18 mos) for day-long LENA (this will be a col vector, so the eventual
    %table looks like this: AgeMonths |  Lday  | L5min  | H5min-AllAd|......
    %                     |    3      |   xx   | .........
    %                     |    6      |   xx   | ..........
    %                     |    9      |   xx   | .........
...
for i = 1:numel(DatasetType)
    FileNumTab.Properties.VariableNames{i+1} = strcat('NumFiles_',DatasetType{i}); %rename third column 
end 
...
if FileNumTab.NumFiles_ValData(end-1) ~= 0
