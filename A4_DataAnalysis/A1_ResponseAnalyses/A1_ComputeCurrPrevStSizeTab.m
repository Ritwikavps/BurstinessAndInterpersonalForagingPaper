...
%Inputs: - ZscoreDataPath: path with z-scored acoustics and time series data
    ...
    p = parpool(numel(OtherType)); %open paralle pool
    
    parfor i = 1:numel(OtherType) %loop for OtherType (since the index for each OtherType speaker tag serves as the index for the corresponding Speaker and responder type, we only need
