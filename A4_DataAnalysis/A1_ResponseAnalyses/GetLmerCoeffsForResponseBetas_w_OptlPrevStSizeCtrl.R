...
#---------------------------------------------------------------------------------------------------------------------------------------------------------------------------
GetRespEff_w_PrevStSiCtrl <- function(FilesToLoad,RespToSpkr){
...
WriteOpToFile_RespEffBetas <- function(WorkingDir,FilePattern,DataType){
  ...
  for (RespType in c('ANRespToCHNSP','CHNSPRespToAN')){ #go through the response-to-speaker types
    ...
    wCtrl_Fname <- strcat(DataType,strcat('_',strcat(RespType,'_RespEff_W_PrevStSizCtrl_DurLogZ_VarsScaleLog_CorpusLvl.csv'))) #get file names to save
    woCtrl_Fname <- strcat(DataType,strcat('_',strcat(RespType,'_RespEff_NoPrevStSizCtrl_DurLogZ_VarsScaleLog_CorpusLvl.csv')))
    
    #WITH prev step size control
    Stats_wStSiCtrl <- GetRespEff_w_PrevStSiCtrl(FilesToLoad,RespType) #Get rec level response effect and prev step size effect stats (these are betas)
