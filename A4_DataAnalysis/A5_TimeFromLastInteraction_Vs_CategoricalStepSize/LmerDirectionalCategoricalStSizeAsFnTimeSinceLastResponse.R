#Ritwika VPS
#R script to do stats on directional 'categorical' step sizes since last response  (ie, steps from X -> C are +1 and C -> X are -1, for example)
#That is, we are not only just looking at whether the infant remains in the same state or now, but we are also iterested in the direction of the change
...
for (i in 1:numel(FilesToLoad)){ #go through files
  ...
  if (str_contains(colnames(mydata)[1],'Since',ignore.case = TRUE)){ #if yes, then we are dealing with non intervening step sizes, and only require the first column 
    ...
    YVar = mydata[,2] #get Y variable
      
    TimeSinceVar = mydata$TimeSinceLastResponse #get time variable
      
    LmerModelFormula = lmer((YVar) ~ (1|ID) + scale(log10(TimeSinceVar + (10^(-10)))) + scale(AgeVar)) #do lmer
    ...
    TimeSinceLastResponsePvalue[i] = StatsSummary$coefficients[2,5]
    ...
    YVar = mydata[,3] #get Y variable
    
    TimeToResponseVar = mydata$TimeToResponse
    TimeFromResponseVar = mydata$TimeFromResponse

    LmerModelFormula = lmer((YVar) ~ (1|ID) + scale(log10(TimeToResponseVar + (10^(-10)))) + scale(log10(TimeFromResponseVar + (10^(-10)))) + scale(AgeVar)) 
    StatsSummary = summary(LmerModelFormula) #get summary
    ...
    AgePvalue[i] = StatsSummary$coefficients[4,5]
    TimeToLastResponseEffect[i] = StatsSummary$coefficients[2,1]  
    TimeToLastResponsePvalue[i] = StatsSummary$coefficients[2,5]
    TimeFromLastResponseEffect[i] = StatsSummary$coefficients[3,1]
    TimeFromLastResponsePvalue[i] = StatsSummary$coefficients[3,5]
...
write.csv(StatsResultsTab, file = " TimeSinceLastResponseVsDirectionalCategoricalStSizeStats.csv",row.names=FALSE)
