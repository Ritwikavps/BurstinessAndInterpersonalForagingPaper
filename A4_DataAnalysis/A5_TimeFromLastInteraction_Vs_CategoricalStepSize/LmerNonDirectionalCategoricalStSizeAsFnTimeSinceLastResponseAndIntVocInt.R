...
#(That is, we are only looking at whether the infant remains in the same state or not) with infant age and intervocalisation interval as explanatory variables
...
for (i in 1:numel(FilesToLoad)){ #go through files #
  ...
  InterVocIntVar = log10(mydata$IntervocInt + (10^(-10)))
  ...
  if (str_contains(colnames(mydata)[1],'Since',ignore.case = TRUE)){ #if yes, then we are dealing with non intervening step sizes, and only require the first column 
    ...
    LmerModelFormula = glmer((YVar) ~ (1|ID) + scale(log10(TimeSinceVar + (10^(-10)))) + scale(AgeVar) + scale(InterVocIntVar), family=binomial()) #do lmer
    ...
    InterVocIntEffect[i] = StatsSummary$coefficients[4,1]
    InterVocIntPvalue[i] = StatsSummary$coefficients[4,4]
    ...
    LmerModelFormula = glmer((YVar) ~ (1|ID) + scale(log10(TimeToResponseVar + (10^(-10)))) + scale(log10(TimeFromResponseVar + (10^(-10)))) + scale(AgeVar) + scale(InterVocIntVar), family=binomial()) 
    StatsSummary = summary(LmerModelFormula) #get summary
    ...
    InterVocIntEffect[i] = StatsSummary$coefficients[5,1]
    InterVocIntPvalue[i] = StatsSummary$coefficients[5,4]
...
write.csv(StatsResultsTab, file = " TimeSinceLastResponseAndIntVocIntVsNonDirectionalCategoricalStSizeStats.csv",row.names=FALSE)
