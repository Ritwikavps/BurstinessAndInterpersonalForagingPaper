...
%This script looks at whether there are correlations between transitions between X and C type infant vocs and
...
%LENA.
%In this script, we stick to case i) and execute case ii) in its own file. 
...
for i = 1:numel(ZscoreFiles)
    ...
    %pick out CHNSP and all AN vocs ONLY
    ...
    AnnotationLabel = TableRead.Annotation(contains(TableRead.Annotation,{'T','U','N','X','C'}),:);
    ...
    if ~isempty(SpeakerLabels(contains(SpeakerLabels,'AN'))) %make sure there are OTHER type vocs
        [NonInterveningStruct,InterveningStruct] = GetCategoricalStepSizeVsTimeSinceLastResponse(StartTime,EndTime,SpeakerLabels,AnnotationLabel,...
                                            SectionNumVec,'C',ChildID,ChildAgeDays,ChildAgeMonths); %Type 1 is 'C' vocs (see function for details)
...
writetable(Chn_NonInterveningCategoricalStSizeTab,'ChnXvsC_AnTUN_NonInterveningCategoricalStSize_HumLabel.csv')
writetable(Chn_InterveningCategoricalStSizeTab,'ChnXvsC_AnTUN_InterveningCategoricalStSize_HumLabel.csv')
