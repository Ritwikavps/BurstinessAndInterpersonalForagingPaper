function [CurrPrevStepSizeTab, TotalMergeCt] = Get_CurrPrevStepSizeTab(ZscoreTab,SpkrType, OtherType, NAType, ResponseWindow, IviOnly)
...
%get step size tab
[DistTab, TotalMergeCt] = GetStepSizeTab(ZscoreTab,SpkrType,OtherType,NAType,ResponseWindow,IviOnly);

if IviOnly == 0 %set number of columns and variable names for output table
    OpNumCols = 13; %if looking at acoustics AND Ivi (extra cols: pitch, amp, duration, 2d, and 3d steps)
    ...
    FinalVarNames = {'CurrInterVocInt','Response','PrevInterVocInt'};                                                 
end
...
if IviOnly == 0 %set number of columns and variable names for output table
    ...
    TabVarNames_Curr  = [TabVarNames_Curr,'Response']; %add Response as an additional var name to the current vars table    
