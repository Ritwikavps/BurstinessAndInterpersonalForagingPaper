GetCurrPrevStSiLmerOp <- function(FilesToLoad,RespToSpkr){
  ...
  for (i in FilesToLoad){
    ...
    if (str_contains(i,RespToSpkr)){
      
      DataTab = read_csv(i)
      attach(DataTab)
      
      RespWindowVal = gsub('s.csv','',gsub('.*_','',i))
      
      for (k in c(3, 6, 9, 18)){
        
        SubsetTab = filter(DataTab, AgeMonths == k) #subset for age in months
      
        Age = SubsetTab$AgeDays
        ID = as_factor(SubsetTab$InfantID)
        Response = as_factor(SubsetTab$Response)
        
        for (j in 1:6){
          
          #print(j)
          
          CurrVar = log10(SubsetTab[,j] + (10^-10))
          ...
          LmMdl_Resp = lm(scale(ResidVar) ~ Response)
          ...
          if (str_contains(colnames(SubsetTab)[j],'DistDuration')){
            
            CurrVar = log10(abs(SubsetTab[,j]) + (10^-10))
            PrevVar = log10(abs(SubsetTab[,j+8]) + (10^-10))
