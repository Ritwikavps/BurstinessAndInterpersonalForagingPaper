...
GetAgeEffectsOnPrevStBeta_IviOnly <-function(InputTabs){
  ...
  DataType <- c(); RespType <- c(); RespWindow_s_Temp <- c()
  ...
  RespWinsToCheck <- c(0.5,1,2,5,10) #response windows to do age analysis on
  
  for (i in InputTabs){
    
    DataTab = read_csv(i); attach(DataTab) #read in table
    
    for (respwin in RespWinsToCheck){
      
      Ctr <- Ctr + 1 #increment counter variable
      SubsetTab <- filter(DataTab, StepType == 'InterVocInt' & RespWindow_s == respwin) #we only need one response window
  ...
  RespWindow_s <- RespWindow_s_Temp #recast
  OpTab <- tibble(DataType,RespType,RespWindow_s,
