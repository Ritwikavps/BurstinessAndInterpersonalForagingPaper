...
GetRespEff_w_PrevStSiCtrl_100sCutOff <- function(FilesToLoad,RespToSpkr){
  ...
  StepVar <- c(); AgeMnth <- c(); RespWindow <- c(); TailOrNo <- c() #initialise vectors to iteratively store output in
  ...
  BeforeOrAfter100IvI <- c('Before','After')
  ...
  for (i in FilesToLoad){ #go through file list
    if (str_contains(i,RespToSpkr)){ #check if file name has the target string
      ...
      for (k in c(3, 6, 9, 18)){ #do analyses separately for different ages
        for (IviCutOff in BeforeOrAfter100IvI){
          
          if (strcmpi(IviCutOff,'Before')){ #implement testing for current Ivis less than 100s and greater than 100s (less than 100s is about the order of mag of Ivis in the validation 
            #dataset, while after 100s is only in the LENA daylong set, so this is the 'tail', so to say
            FirstSubSet <- filter(DataTab, CurrInterVocInt <= 100)
          }else if (strcmpi(IviCutOff,'After')){
            FirstSubSet <- filter(DataTab, CurrInterVocInt > 100)
          }
        
          SubsetTab <- filter(FirstSubSet, AgeMonths == k) #subset for age in months
          ...
          for (j in 2){#1:6
            ...
            #store results
            StepVar[Ctr] <- gsub('Curr','',colnames(SubsetTab)[j]); RespWindow[Ctr] <- RespWindowVal; AgeMnth[Ctr] <- k; TailOrNo[Ctr] <- IviCutOff
