...
`A4_GetDataToPlotAnalysesStepsDataViz.R` performs the previous IEI control step at the recording level on all analysed data to get recording level residuals from the previous IEI control (i.e., for each daylong recording for the LENA day-long data, and for each validation data file for the validation data). The resulting data can be used for plotting and visualisation purposes as needed. Requires `GetPrevStCtrlResids_RecLvl.R`. However, as it stands, the output table from this script is only used to get the intercepts and slops for the previous IEI regression (used in the relevant main Fig. 1 plot) by grouping LENA daylong data current and previous IEIs at the age group level. So, while there are more efficient ways of doing this, this is how the pipeline ended up forming, so this script has to be run for downstream scripts. 
