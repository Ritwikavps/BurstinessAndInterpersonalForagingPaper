...
FilesToLoad = dir(path=".", pattern="*CategoricalStSizvsInterVocInt.csv*")
...
for (i in 1:numel(FilesToLoad)){ #go through the files
  ...
  LmerModelFormula = glmer((Yvar) ~ (1|ID) + scale(Xvar) + poly(AgeVar,2,raw = TRUE), family=binomial()) #do lmer
  StatsSummary = summary(LmerModelFormula) #get summary
  
  Age1Effect[i] = StatsSummary$coefficients[3,1]
  Age1Pvalue[i] = StatsSummary$coefficients[3,4]
  Age2Effect[i] = StatsSummary$coefficients[4,1]
  Age2Pvalue[i] = StatsSummary$coefficients[4,4]
...
StatsResultsTab <- list(as.data.frame(StepSizeType),  as.data.frame(DataType),
                        ...
                        as.data.frame(Age1Effect), as.data.frame(Age1Pvalue),as.data.frame(Age2Effect),as.data.frame(Age2Pvalue))
write.csv(StatsResultsTab, file = "CategoricalStepSizesVsIntervocInterval_UnscaledRawPolyAgeStats.csv",row.names=FALSE)
