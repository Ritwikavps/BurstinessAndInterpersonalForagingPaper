...
%outputs .csv file with acoustic step sizes and inter-voc intervals, as well as infant age and id info
...
for i = 1:numel(DirStruct)
    ...
    ChildVocStruct = ComputeInterVocIntervalAndAcousticSpaceSteps(SectionNumVec(contains(SpeakerLabels,ChildVocString)),Pitch(contains(SpeakerLabels,ChildVocString)),...
                     Amplitude(contains(SpeakerLabels,ChildVocString)),Duration(contains(SpeakerLabels,ChildVocString)),StartTime(contains(SpeakerLabels,ChildVocString)),...
                     EndTime(contains(SpeakerLabels,ChildVocString)),ChildID,ChildAgeDays,ChildAgeMonth);

    if nargin == 7 %(hacky think for HBCVV validation; fix this later, cuz it doesnt have adult data)
