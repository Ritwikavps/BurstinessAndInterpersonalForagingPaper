function [OutputStruct] = ComputeInterVocIntervalAndAcousticSpaceSteps(SectionNum,Pitch,Amplitude,StartTime,EndTime,Id,AgeDays,AgeMonths)

%Ritwika VPS, Sep 2022
%function to compute acoustic space steps (2d and 3d) and intervocalisation
%interval, given pitch, amplitude, duration, start and end times of vocs as
%well as information about section number (for 5 minute segments, this
%would just be the section number, while for daylong LENA data, all vocs
%from the start to end of a subrecording would belong to the same section)

%inputs: - start and end times of vocs; pitch, amplitude, and duration of
    %vocs; and the section number for each voc
%outputs: structure with vectors of 2d (space = pitch and amp) and 3d (additional duration
    %dimension) acoustic space steps, and intervocalisation interval,
    %accounting for the fact that steps between subrecordings or sections
...
Steps2D_vec = []; %iniitialsie outputs (column vector)
Steps3D_vec = [];
...
OutputStruct.Steps2D = Steps2D_vec;
OutputStruct.Steps3D = Steps3D_vec;
