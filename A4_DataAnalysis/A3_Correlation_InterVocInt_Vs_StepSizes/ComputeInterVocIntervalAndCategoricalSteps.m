function [OutputStruct] = ComputeInterVocIntervalAndCategoricalSteps(StartTime,EndTime,AnnotationLabel,SpeakerLabels,SectionNum,Type1,ChildID,ChildAgeDays,ChildAgeMonths,Pitch,Amplitude,Duration)
...
%The inputs are: Start and End times; speaker labels for the recording (or subrecording or 5 minute section), where CHN
    ...
    % by default (see below); and finally infant id, age in days, and age
    % in months. Optionally, acoustic step sizes associated with
    % categorical step sizes can also be computed but we opt not to do that
    % at this time
...
for i = 1:numel(UniqSectionNum) %go through unique section numbers
    ...
    if nargin == 12 %optional acoustic step size computation
        PitchStep = abs(diff(Pitch(SectionNum == UniqSectionNum(i))));
        AmpStep = abs(diff(Amplitude(SectionNum == UniqSectionNum(i))));
        DurationStep = abs(diff(Duration));

        SpaceStep2d_vec = [SpaceStep2d_vec; sqrt(PitchStep.^2 + AmpStep.^2)];
        SpaceStep3d_vec = [SpaceStep3d_vec; sqrt(PitchStep.^2 + AmpStep.^2 + DurationStep.^2)];
    end
end
...
if nargin == 12 %optional acoustic step size computation
    OutputStruct.SpaceSteps2d = SpaceStep2d_vec;
    OutputStruct.SpaceSteps3d = SpaceStep3d_vec;
end
